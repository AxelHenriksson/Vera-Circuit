...
class OBJMesh {
    ...
    companion object {
        ...
        fun parseOBJ(context: Context, objAsset: String, position: Vec3, scale: Float, shader: Shader): Mesh {
            val objString = Utils.getStringFromAsset(context, objAsset)
...
//        fun parseOBJMTL(context: Context, objRes: Int, mtlRes: Int, position: Vec3, scale: Float): Array<Mesh> {
//
//            val objString = Utils.getStringFromResource(context, objRes)
//            val mtlString = Utils.getStringFromResource(context, mtlRes)
//
//            data class Position(val x: Float, val y: Float, val z: Float) { operator fun times(factor: Float) = Position(factor * x, factor * y, factor * z) }
//            data class TexCoord(val u: Float, val v: Float)
//            data class Normal(val x: Float, val y: Float, val z: Float)
//            data class Vertex(val pos: Position, val texCoord: TexCoord, val normal: Normal)
//
